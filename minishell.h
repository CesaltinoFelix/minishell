...
void			execute_pipeline(t_minishell *shell, t_pipeline *cmds,
					int cmd_count);
...
void			copy_replacement_to_input(t_minishell *shell,
					char *updated_input, size_t chars_before_var,
...
void			process_token(const char **input, char *quote, char delimiter,
					size_t *count);
...
int				ft_handle_heredoc(t_minishell *shell, int i,
					char *heredoc_path);
int				is_env_var_match(const char *env_var, const char *key,
					size_t key_len);
...
int				exec_redirection(t_minishell *shell, t_pipeline *cmds,
					int cmd_count);
int				handle_heredoc_child(int fd, t_minishell *shell, int i,
					char *file);
